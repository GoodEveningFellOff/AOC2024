...
static constexpr std::array g_aInput = {
	Input_t{ "Test", "14", "./Resources/C8a_1.txt"},
...
static void SetAntinodeForTwoNodes(ParsedInput_t& vecAntinodes, int64_t iLine1, int64_t iColumn1, int64_t iLine2, int64_t iColumn2)
{
	...
	char c = GetChar(vecAntinodes, iLine1 - iLineOffset, iColumn1 - iColumnOffset);
	if (c == '.')
		vecAntinodes[iLine1 - iLineOffset][iColumn1 - iColumnOffset] = '#';

	c = GetChar(vecAntinodes, iLine2 + iLineOffset, iColumn2 + iColumnOffset);
	if (c == '.')
		vecAntinodes[iLine2 + iLineOffset][iColumn2 + iColumnOffset] = '#';
}
