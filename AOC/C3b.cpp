...
static constexpr std::array g_aInput = {
	Input_t{ "Test", "48", "./Resources/C3b_1.txt"},
	Input_t{ "Real", "???", "./Resources/C3a_2.txt"}
};

using ParsedInput_t = std::vector<std::variant<std::pair<uint64_t, uint64_t>, bool>>;
...
static ParsedInput_t ParseInput(std::string_view svInput)
{
	...
	for (std::string_view::const_iterator itr = svInput.begin(); itr != svInput.end(); ++itr)
	{
		...
		case(','):
			...
			break;
		case('d'):
			ePosition = (ePosition == EParsePosition::Invalid) ? EParsePosition::D : EParsePosition::Invalid;
			break;
		case('o'):
			ePosition = (ePosition == EParsePosition::D) ? EParsePosition::O : EParsePosition::Invalid;
			break;
		case('n'):
			ePosition = (ePosition == EParsePosition::O) ? EParsePosition::N : EParsePosition::Invalid;
			break;
		case('\''):
			ePosition = (ePosition == EParsePosition::N) ? EParsePosition::Apostrophy : EParsePosition::Invalid;
			break;
		case('t'):
			ePosition = (ePosition == EParsePosition::Apostrophy) ? EParsePosition::T : EParsePosition::Invalid;
			break;
		case('('):
			...
			else if (ePosition == EParsePosition::O) {
				bIsDo = true;
				ePosition = EParsePosition::InstructionOpeningParenthesis;
			}
			else if (ePosition == EParsePosition::T) {
				bIsDo = false;
				ePosition = EParsePosition::InstructionOpeningParenthesis;
			}
			else
		...
		case(')'):
			...
			else if (ePosition == EParsePosition::InstructionOpeningParenthesis)
				vecReturned.emplace_back(bIsDo);
			ePosition = EParsePosition::Invalid;
			break;
		case('0'):
		case('1'):
...
static uint64_t Solve(std::string_view svPath)
{
	...
	bool bIsDo = true;
	uint64_t iTotal{};
	for (const auto& variantData : vecInput)
	{
		if (std::holds_alternative<bool>(variantData))
		{
			bIsDo = std::get<bool>(variantData);
			continue;
		}
		...
		const auto& pairData = std::get<std::pair<uint64_t, uint64_t>>(variantData);
